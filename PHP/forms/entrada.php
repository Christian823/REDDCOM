<?php
    ...
    if ($count > 0) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $sqlInsert = "INSERT INTO registros (proyecto, equipo, anexo, comentario, horaEntrada, fecha, firstName, lastName, user, latitud, longitud ) 
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";



            $stmtInsert = $conn->prepare($sqlInsert);
            $stmtInsert->bind_param("sssssssssss", $proyecto, $equipoSeleccionadoString, $anexo_contenido, $comentarios, $horaEntrada, $fecha, $nombre, $apellidos, $user,$latitud, $longitud);
